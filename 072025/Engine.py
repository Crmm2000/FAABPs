...
import utils as u #contains helper functions like periodic boundary condition, experiment details
...
def simulate_system(radius_repulsive_potential, n_particles, v_0, curvity, persistence_length=5, t_end = 100, dt = 1e-2, boundary = 20, stiffness = 1, interacting = False, mode = None, N = 2, mode_particles = 'ABP'):
    ...
    # setting parameters -> adjustable to heterogeneous particles: np.random.rand(n_particles, 1) -> can go into one big array
    ...
    #sets eperiment up if there is one, overwrites previously set initial conditions
    if mode != None:
    ...
    for i in tqdm(range(n-1)):  # Loop through timesteps
        ...
        #Bookkeeping probabilities -> can set this to one simple function
        if mode == 'Iso_oreo':
    ...
    #hypersphere
    space_repulsive_potentials = np.sum((np.pi**(N/2)/math.gamma(N/2 + 1)) * b[:3]**N)
